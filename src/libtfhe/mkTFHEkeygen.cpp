...
// 重写的方法，固定了a，CRS
EXPORT void MKRLweKeyGen22(MKRLweKey *result,MKRLweKey *crs) {
    const int32_t parties = result->MKparams->parties;
    ...
    for (int j = 0; j < dg; ++j)
    {
        //result->Pkey[parties*dg + j]=crs->Pkey[dg + j];
        for (int l = 0; l < N; ++l)
        {
            result->Pkey[parties*dg + j].coefsT[l]=crs->Pkey[dg + j].coefsT[l];
        }    
    }
...
EXPORT void MKRLweKeyGen(MKRLweKey *result) {
    ...
    for (int j = 0; j < dg; ++j)
    {
        torusPolynomialUniform(&result->Pkey[dg + j]);
    }
}
